...
public class ContextLoader {
	...
	static {
		...
		//读取默认配置文件“ContextLoader.properties”的属性信息，
		//这个文件在源码的resources目录下
		try {
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		...
		try {
			...
			if (this.context == null) {
				//重要!
				//返回XmlWebApplicationContext实例,Web环境下的Spring IoC容器
				//只是实例化了XmlWebApplicationContext，并没有配置任何东西
				this.context = createWebApplicationContext(servletContext);
			}
			//继承情况：XmlWebApplicationContext→ConfigurableWebApplicationContext→WebApplicationContext
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					if (cwac.getParent() == null) {
						...
						//通过servletContext获取web.xml的配置信息；
						//在web.xml中配置了<context-param>的parentContextKey才会指定父级应用
						//父级应用就是parentContextKey
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		...
		if (contextClassName != null) {
			...
			//默认的class名为：XmlWebApplicationContext
			//是在ContextLoader.properties文件中配置的
			//这个文件的内容：org.springframework.web.context.WebApplicationContext=org.springframework.web.context.support.XmlWebApplicationContext
